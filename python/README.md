...
1. **Use the JavaScript SDK via subprocess** (Recommended for now)
   ...
   - This repo includes a bridge at `scripts/clob_bridge.mjs`
   - Install JS deps with: `npm install` (Node 18+ recommended)
   - The Python CLOB wrapper calls the bridge for order execution
   - Optional: set `CLOB_SIGNATURE_TYPE=POLY_PROXY` (or `POLY_GNOSIS_SAFE`) if your proxy wallet is a contract

...
impl MarketCaches {
    ...
    pub fn load_all(&self) -> CacheLoadResult {
        ...
        // Load Ligue 1 (soccer) tokens cache
    ...
    /// Check if token is tennis (ATP) market
    #[inline]
    ...
    /// Check if token is soccer (Ligue 1) market
    #[inline]
    ...
    /// Get tennis buffer for token (0.01 if tennis, 0.0 otherwise)
    #[inline]
    pub fn get_tennis_buffer(&self, token_id: &str) -> f64 {
        if self.is_tennis_token(token_id) { TENNIS_BUFFER } else { 0.0 }
    }

    /// Get soccer buffer for token (0.01 if soccer, 0.0 otherwise)
    #[inline]
    pub fn get_soccer_buffer(&self, token_id: &str) -> f64 {
        if self.is_soccer_token(token_id) { SOCCER_BUFFER } else { 0.0 }
    }
...
/// Get tennis buffer for a token (convenience function)
#[inline]
pub fn get_tennis_token_buffer(token_id: &str) -> f64 {
    global_caches().get_tennis_buffer(token_id)
}

/// Get soccer buffer for a token (convenience function)
#[inline]
pub fn get_soccer_token_buffer(token_id: &str) -> f64 {
    global_caches().get_soccer_buffer(token_id)
}

/// Backwards-compatible ATP buffer access
#[inline]
pub fn get_atp_token_buffer(token_id: &str) -> f64 {
    get_tennis_token_buffer(token_id)
}

/// Backwards-compatible Ligue 1 buffer access
#[inline]
pub fn get_ligue1_token_buffer(token_id: &str) -> f64 {
    get_soccer_token_buffer(token_id)
}
...
/// Backwards-compatible ATP token check
#[inline]
pub fn is_atp_token(token_id: &str) -> bool {
    global_caches().is_tennis_token(token_id)
}

/// Backwards-compatible Ligue 1 token check
#[inline]
pub fn is_ligue1_token(token_id: &str) -> bool {
...
mod tests {
    ...
    fn test_non_existent_returns_zero_buffer() {
        let caches = MarketCaches::new();
        assert_eq!(caches.get_tennis_buffer("nonexistent"), 0.0);
        assert_eq!(caches.get_soccer_buffer("nonexistent"), 0.0);
    }
}
